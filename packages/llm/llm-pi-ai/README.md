...
How a thinking level travels — `reasoning_effort` alone, DeepSeek's `thinking: {type}` plus effort, z.ai's `thinking` object, and so on — is pi-ai's `compat.thinkingFormat`, which pi-ai guesses from the endpoint URL; a private gateway's URL says nothing, so a DeepSeek-dialect gateway would be spoken to in the OpenAI dialect with no way to correct it. Beyond reasoning dispatch, OpenAI-compatible gateways disagree on a small batch of other protocol facts pi-ai also guesses by URL: whether the system prompt uses `system` or `developer` (`supportsDeveloperRole` — Volcengine Ark accepts only the former), which field caps output tokens (`maxTokensField`: `max_tokens` or `max_completion_tokens`), whether `store` is accepted (`supportsStore`), whether tool results must carry a `name` (`requiresToolResultName`), whether a user message after tool results needs an assistant message in between (`requiresAssistantAfterToolResult`), whether thinking blocks must be sent as `<thinking>`-delimited text (`requiresThinkingAsText`), and whether replayed assistant messages must echo an empty `reasoning_content` while reasoning is enabled (`requiresReasoningContentOnAssistantMessages`). These nine switches are configurable on the route (its models' default) and per model (winning per field), resolving model → route → gateway preset → installed catalog entry → pi-ai's URL-derived guess; setting a route-level switch shadows the catalog entry's value for every model on the route, and there is no spelling for handing a field back to the catalog short of restating its value. `thinkingFormat` accepts pi-ai's dispatchable formats except the two `chat-template` variants, which need `chatTemplateKwargs` this configuration does not expose. All nine switches exist only on `openai-completions` — the other protocols carry their reasoning shape in the protocol itself — so a model-level switch elsewhere fails resolution, a route-level one or a gateway preset skips models of other protocols, and a route with no `openai-completions` model at all refuses a route-level switch. The rest of pi-ai's compat surface (grammar tools, strict mode, cache-control formats, session affinity, routing dialects) stays auto-detected and is deliberately not configurable here.

**Gateway presets** hand these nine switches to plugins. The optional `llm-gateway-compat-presets` service is a registry from endpoint hostname to compat facts: any plugin can `ctx.provide` one, and the adapter asks it per route about the hostname the route actually points at, landing a match under explicit `compat` and over the catalog entry — it describes the endpoint, so it beats a catalog assumption a repointed route no longer speaks for. The shipped composition mounts [`dsh-llm-gateway-presets`](https://www.npmjs.com/package/@deepseek-ai/dsh-llm-gateway-presets), which provides the service seeded with the empirically verified Volcengine Ark entry (`*.volces.com` → `supportsDeveloperRole: false`); deployments extend or replace entries through its `presets` config, and community gateway knowledge lands as data contributions to that package or as plugins providing their own registry, so a new channel never requires core changes. Presets pin compat facts only — they cannot declare reasoning levels, which remain `reasoningEfforts`'s per-model capability.
...
With `probeCapabilities` set, the interrogation additionally probes the models named by `models` (or every listed one when omitted) for their reasoning capability through a few small `openai-completions` requests, and each model's facts ride its `reasoning` — the selectable thinking levels as `{ level: wire spelling }`, whether the endpoint accepted the `developer` system role, the `thinkingFormat` the endpoint needs when the OpenAI dialect is refused, the output-cap field it accepts (`max_tokens` or `max_completion_tokens`), or a short `failed` reason. Only `openai-completions` has the probed chat shape; any other protocol keeps the listing answer with no facts. The verdict is always the status code — 2xx accepts, 400 refuses the parameter, anything else fails that model's probe without failing the listing — so a refused parameter can never be mistaken for an unreachable endpoint, and the adopting surface writes the confirmed levels into `reasoningEfforts` and the dialect facts into `compat` instead of asking the user to know them.
